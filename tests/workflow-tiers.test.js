...
// Test 10: Update mode removes qa-architect-generated duplicates but preserves user workflows
;(() => {
  console.log(
    'Test 10: Update mode respects workflow provenance on duplicate cleanup'
  )
  const testDir = createTempGitRepo()
  ...
  try {
    ...
    // qa-architect-generated ci.yml (contains marker) — should be deleted
    const qaaCiPath = path.join(workflowDir, 'ci.yml')
    fs.writeFileSync(
      qaaCiPath,
      'name: CI\n# Generated by create-qa-architect\non: [push]\njobs: { test: { runs-on: ubuntu-latest, steps: [] } }\n'
    )

    // User-authored test.yml (no marker) — should be preserved
    const userTestPath = path.join(workflowDir, 'test.yml')
    fs.writeFileSync(
      userTestPath,
      'name: My Tests\non: [push]\njobs: { test: { runs-on: ubuntu-latest, steps: [] } }\n'
    )
    ...
    assert(
      !fs.existsSync(qaaCiPath),
      'Should auto-remove qa-architect-generated ci.yml'
    )
    assert(
      fs.existsSync(userTestPath),
      'Should preserve user-authored test.yml (no qa-architect marker)'
    )
    assert(
