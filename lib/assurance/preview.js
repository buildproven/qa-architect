...
function redactEvidenceValue(value, redactions = []) {
  return redactions.reduce(
    (redacted, sensitive) =>
      sensitive ? redacted.replaceAll(sensitive, '[redacted]') : redacted,
    value
  )
}

function requestIdentity(url, method, redactions) {
  return {
    method,
    origin: url.origin,
    path: redactEvidenceValue(url.pathname, redactions),
...
async function readBoundedBody(response) {
  ...
  const captured = Buffer.allocUnsafe(MAX_BODY_BYTES)
  let size = 0
  let truncated = false
  try {
    while (size < MAX_BODY_BYTES) {
      ...
      const captureBytes = Math.min(value.byteLength, remaining)
      Buffer.from(value).copy(captured, size, 0, captureBytes)
      size += captureBytes
      if (value.byteLength > captureBytes) {
        truncated = true
        await reader.cancel()
        break
      }
    }
    if (size === MAX_BODY_BYTES && !truncated) {
  ...
  return { bytes: captured.subarray(0, size), truncated }
}
...
function stateRequest(spec, token, fixtureId, resourceId) {
  return {
    ...
    evidenceRedactions: [fixtureId, resourceId].filter(Boolean),
