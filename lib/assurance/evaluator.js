...
const { semanticErrors } = require('./policy')
...
function alignRequiredness(current, isRequired) {
...
function normalizeChecks(checks, supported, required, reasons) {
  ...
  for (const id of supported) {
    ...
    alignRequiredness(current, isRequired)
...
function effectivePolicy(input, now, reasons) {
  ...
  if (input.valid === false) {
    for (const error of input.errors || ['Malformed assurance policy']) {
      reasons.push(reason('incomplete', 'policy.malformed', error))
    }
    return DEFAULT_POLICY
  }
  return input.policy || input
}
...
function evaluateAssurance({
  ...
  const required = normalizeScope(requiredChecks, 'required', reasons)

  for (const [id, value] of Object.entries(policy.requiredChecks || {})) {
    if (value === true && CHECK_ID_SET.has(id)) {
