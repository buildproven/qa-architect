...
const PROJECT_CONFIGS = {
  ...
  webapp: {
    ...
    testCommands: {
      ...
      comprehensive:
        "npm run test:medium 2>/dev/null || npm run test -- --exclude 'tests/e2e/**' 2>/dev/null || npm test",
      medium:
        "npm run test:medium 2>/dev/null || npm run test -- --exclude 'tests/e2e/**'",
  ...
  api: {
    ...
    testCommands: {
      ...
      comprehensive:
        'npm run test:medium 2>/dev/null || npm run test:fast 2>/dev/null || npm test',
      medium:
        "npm run test:medium 2>/dev/null || npm run test -- --exclude 'tests/integration/**'",
      fast: 'npm run test:fast 2>/dev/null || npm run test:unit 2>/dev/null || npm test',
      minimal: 'npm run lint && npm run format:check',
    },
    detection: projectPath => {
...
function generateSmartPrePushHook() {
  return `# Git supplies one update per line on stdin. A push containing only
# deletions changes no code, so running tests and security scans cannot add
# assurance. Keep ordinary and new-branch pushes fail-closed.
PUSH_UPDATES=""
if [ ! -t 0 ]; then
  PUSH_UPDATES=$(cat)
fi
if [ -n "$PUSH_UPDATES" ] && printf '%s\\n' "$PUSH_UPDATES" | awk '
  NF { saw = 1; if ($2 !~ /^0+$/) non_deletion = 1 }
  END { exit !(saw && !non_deletion) }
'; then
  echo "✅ Remote branch deletion only — no code to validate."
  exit 0
fi
...
# 2. Dependency audit (production deps only)
# Dev-tooling CVEs don't ship to users, and blocking on them stalls pushes on
# advisories with no consumer-side fix. Matches quality.yml's blocking gate.
echo "  → Checking production dependencies..."
# Detect package manager and use appropriate audit command
if [ -f "pnpm-lock.yaml" ]; then
  pnpm audit --audit-level=high --prod || {
    echo "❌ Vulnerable production dependencies found! Run 'pnpm audit --fix' to resolve."
    exit 1
  }
elif [ -f "yarn.lock" ]; then
  yarn audit --level high --groups dependencies || {
    echo "❌ Vulnerable production dependencies found! Run 'yarn audit fix' to resolve."
    exit 1
  }
else
  npm audit --audit-level=high --omit=dev || {
    echo "❌ Vulnerable production dependencies found! Run 'npm audit fix' to resolve."
    exit 1
  }
fi
...
function getTestTierScripts() {
  return {
    // Vitest 4 dropped the `basic` reporter (valid names: default, agent,
    // minimal, blob, verbose, dot, json, tap, tap-flat, junit, tree,
    // hanging-process, github-actions) and never had a Jest-style
    // --testPathIgnorePatterns flag — it uses --exclude with glob patterns.
    // Both scripts crashed on first real invocation before this fix.
    'test:fast': 'vitest run --reporter=dot --coverage=false',
    'test:medium':
      "vitest run --reporter=dot --exclude 'tests/e2e/**' --exclude 'tests/integration/**'",
    'test:comprehensive':
