...
function installBlocker(projectPath, profile, installs) {
  if (!installs.includes(null)) return null
...
function buildPolicy(projectPath, options = {}) {
  ...
  const unsupportedInstall = installBlocker(projectPath, profile, installs)
  if (unsupportedInstall) blockers.push(unsupportedInstall)
