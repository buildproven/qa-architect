'use strict'

const fs = require('fs')
const path = require('path')
const crypto = require('crypto')
const yaml = require('js-yaml')
const { detectProjectProfile } = require('./project-profile')

const POLICY_PATH = '.buildproven/test-impact.json'
const WORKFLOW_PATH = '.github/workflows/test-impact.yml'
const CHECKOUT_SHA = '3d3c42e5aac5ba805825da76410c181273ba90b1'
const SETUP_NODE_SHA = '820762786026740c76f36085b0efc47a31fe5020'
const SHA_PATTERN = /^[0-9a-f]{40}$/
const NODE_FILE = /\.(?:[cm]?js)$/
const NODE_TEST =
  /(?:^|\/)(?:tests?|spec|__tests__)(?:\/|$)|\.(?:test|spec)\.[cm]?js$/
const SCAN_IGNORES = new Set([
  '.buildproven',
  '.git',
  '.github',
  'build',
  'coverage',
  'dist',
  'node_modules',
])
const RUNNER_PATTERNS = {
  jest: /(^|[^A-Za-z0-9_-])jest([^A-Za-z0-9_-]|$)/,
  node: /(^|[\s;&|()])node(?:\s|$)/,
  vitest: /(^|[^A-Za-z0-9_-])vitest([^A-Za-z0-9_-]|$)/,
}
const LOCAL_IMPORT =
  /(?:require\s*\(|from\s+|import\s+(?!\()|import\s*\()\s*['"](\.[^'"]+)['"]/g

function selectJsRunner(testCommand, declaredRunners) {
  const invoked = declaredRunners.filter(runner =>
    RUNNER_PATTERNS[runner].test(testCommand)
  )
  if (invoked.length === 1) return invoked[0]
  if (invoked.length === 0 && RUNNER_PATTERNS.node.test(testCommand))
    return 'node'
  if (declaredRunners.length === 1) return declaredRunners[0]
  return 'none'
}

function readJson(file) {
  return JSON.parse(fs.readFileSync(file, 'utf8'))
}

function declaredDependencies(projectPath) {
  const packagePath = path.join(projectPath, 'package.json')
  if (!fs.existsSync(packagePath)) return {}
  const pkg = readJson(packagePath)
  return { ...(pkg.dependencies || {}), ...(pkg.devDependencies || {}) }
}

function declaredTestCommand(projectPath, scriptName) {
  const packagePath = path.join(projectPath, 'package.json')
  if (!scriptName || !fs.existsSync(packagePath)) return ''
  return readJson(packagePath).scripts?.[scriptName] || ''
}

function commandForScript(profile, script) {
  if (!script) return null
  if (profile.packageManager === 'npm') {
    return { executable: 'npm', args: ['run', script] }
  }
  if (profile.packageManager === 'pnpm') {
    return { executable: 'pnpm', args: ['run', script] }
  }
  if (profile.packageManager === 'yarn') {
    return { executable: 'yarn', args: [script] }
  }
  if (profile.packageManager === 'bun') {
    return { executable: 'bun', args: ['run', script] }
  }
  return null
}

function fileContains(projectPath, file, pattern) {
  const target = path.join(projectPath, file)
  return fs.existsSync(target) && pattern.test(fs.readFileSync(target, 'utf8'))
}

function hasPythonTests(projectPath) {
  if (fs.existsSync(path.join(projectPath, 'pytest.ini'))) return true
  if (fileContains(projectPath, 'pyproject.toml', /\[tool\.pytest\b/))
    return true
  if (fileContains(projectPath, 'setup.cfg', /\[tool:pytest\]/)) return true
  if (fileContains(projectPath, 'tox.ini', /\bpytest\b/)) return true
  const requirements = fs
    .readdirSync(projectPath)
    .filter(file => /^requirements.*\.txt$/.test(file))
  return requirements.some(file =>
    fileContains(projectPath, file, /^pytest(?:\b|[<=>])/m)
  )
}

function installCommands(projectPath, profile, python) {
  const commands = []
  if (fs.existsSync(path.join(projectPath, 'package.json'))) {
    const safeInstall = {
      npm: 'npm ci --ignore-scripts',
      pnpm: 'pnpm install --frozen-lockfile --ignore-scripts',
      yarn: 'yarn install --immutable --mode=skip-build',
      bun: 'bun install --frozen-lockfile --ignore-scripts',
    }[profile.packageManager]
    commands.push(safeInstall || null)
  }
  if (python) {
    if (fs.existsSync(path.join(projectPath, 'requirements.txt'))) {
      commands.push('python -m pip install -r requirements.txt')
    } else {
      commands.push(null)
    }
  }
  return commands
}

function packageManagerSetup(profile) {
  if (profile.packageManager === 'npm') {
    return profile.packageManagerVersion
      ? [`npm install --global npm@${profile.packageManagerVersion}`]
      : []
  }
  if (profile.packageManager === 'pnpm' || profile.packageManager === 'yarn') {
    return [
      'corepack enable',
      `corepack prepare ${profile.packageManager}@${profile.packageManagerVersion} --activate`,
    ]
  }
  return null
}

function workflowInventory(projectPath) {
  const directory = path.join(projectPath, '.github', 'workflows')
  if (!fs.existsSync(directory)) return []
  return fs
    .readdirSync(directory)
    .filter(file => /\.ya?ml$/.test(file))
    .sort()
    .map(file => {
      const relativePath = `.github/workflows/${file}`
      const content = fs.readFileSync(path.join(directory, file), 'utf8')
      const parsed = yaml.load(content) || {}
      const jobs = Object.entries(parsed.jobs || {}).map(([id, job]) => ({
        id,
        name: job && typeof job === 'object' ? job.name || id : id,
      }))
      return { path: relativePath, jobs }
    })
}

function walkFiles(projectPath, relative = '') {
  const directory = path.join(projectPath, relative)
  const files = []
  for (const entry of fs.readdirSync(directory, { withFileTypes: true })) {
    if (entry.isSymbolicLink()) continue
    const child = relative ? `${relative}/${entry.name}` : entry.name
    if (entry.isDirectory()) {
      if (!SCAN_IGNORES.has(entry.name)) {
        files.push(...walkFiles(projectPath, child))
      }
    } else if (entry.isFile()) {
      files.push(child)
    }
  }
  return files
}

function resolveLocalControl(projectPath, importer, request) {
  const base = path.posix.normalize(
    path.posix.join(path.posix.dirname(importer), request)
  )
  const candidates = [
    base,
    `${base}.js`,
    `${base}.cjs`,
    `${base}.mjs`,
    `${base}.json`,
    `${base}/index.js`,
    `${base}/index.cjs`,
    `${base}/index.mjs`,
  ]
  return candidates.find(candidate => {
    const target = path.join(projectPath, candidate)
    return fs.existsSync(target) && fs.statSync(target).isFile()
  })
}

function isRunnerControl(file, jsRunner, python) {
  if (jsRunner === 'vitest' && /^vitest\.config\.[^/]+$/.test(file)) return true
  if (jsRunner === 'jest' && /^jest\.config\.[^/]+$/.test(file)) return true
  return (
    python && /^(?:pytest\.ini|pyproject\.toml|setup\.cfg|tox\.ini)$/.test(file)
  )
}

function runnerControlFiles(projectPath, jsRunner, python) {
  const controls = new Set(
    walkFiles(projectPath).filter(file =>
      isRunnerControl(file, jsRunner, python)
    )
  )
  const pending = [...controls]
  while (pending.length > 0) {
    const importer = pending.shift()
    const content = fs.readFileSync(path.join(projectPath, importer), 'utf8')
    for (const match of content.matchAll(LOCAL_IMPORT)) {
      const resolved = resolveLocalControl(projectPath, importer, match[1])
      if (resolved && !controls.has(resolved)) {
        controls.add(resolved)
        pending.push(resolved)
      }
    }
  }
  return [...controls].sort()
}

function testStem(file) {
  return path.basename(file).replace(/\.(?:test|spec)?\.[cm]?js$/, '')
}

function sourceStem(file) {
  return path.basename(file).replace(/\.[cm]?js$/, '')
}

function plainNodeMappings(projectPath) {
  const files = walkFiles(projectPath).filter(file => NODE_FILE.test(file))
  const tests = files.filter(file => NODE_TEST.test(file)).sort()
  const sources = files.filter(file => !NODE_TEST.test(file)).sort()
  return sources.flatMap(source => {
    const matchingTests = tests.filter(
      test => testStem(test) === sourceStem(source)
    )
    if (matchingTests.length === 0) return []
    return [
      {
        paths: [source],
        commands: matchingTests.map(test => ({
          executable: 'node',
          args: [test],
        })),
      },
    ]
  })
}

function nodeMappingReview(projectPath, jsRunner) {
  const suggestions = jsRunner === 'node' ? plainNodeMappings(projectPath) : []
  return {
    suggestions,
    blocker:
      suggestions.length > 0
        ? 'Plain Node source mappings need repository-owned dependency or coverage evidence. Review inventory.mappingSuggestions and add complete mappings.'
        : null,
  }
}

function buildPolicy(projectPath) {
  const profile = detectProjectProfile(projectPath)
  const dependencies = declaredDependencies(projectPath)
  const python = hasPythonTests(projectPath)
  const installs = installCommands(projectPath, profile, python)
  const managerSetup = packageManagerSetup(profile)
  const completeCommands = []
  const testCommand = commandForScript(profile, profile.scripts.test)
  if (testCommand) completeCommands.push(testCommand)
  if (python) {
    completeCommands.push({ executable: 'pytest', args: [] })
  }

  const declaredJsRunners = ['vitest', 'jest'].filter(
    runner => dependencies[runner]
  )
  const testCommandText = declaredTestCommand(projectPath, profile.scripts.test)
  const jsRunner = selectJsRunner(testCommandText, declaredJsRunners)

  const auditPaths = [
    POLICY_PATH,
    WORKFLOW_PATH,
    'package.json',
    'package-lock.json',
    'pnpm-lock.yaml',
    'yarn.lock',
    'bun.lock',
    'bun.lockb',
    'pyproject.toml',
    'pytest.ini',
    'setup.cfg',
    'tox.ini',
    'requirements*.txt',
    'vitest.config.*',
    'jest.config.*',
  ]
  const policy = {
    version: 1,
    jsRunner,
    mappings: [],
    audits:
      completeCommands.length === 0
        ? []
        : [
            {
              paths: auditPaths,
              reason:
                'test dependency, runner, workflow, or selector policy changed',
              commands: completeCommands,
            },
          ],
  }

  const blockers = []
  if (completeCommands.length === 0) {
    blockers.push('No declared complete test command was found.')
  }
  if (profile.hasTests && jsRunner === 'none' && profile.scripts.test) {
    blockers.push(
      declaredJsRunners.length > 1
        ? 'JavaScript tests declare both Jest and Vitest. Select one runner or add repository-owned path mappings.'
        : 'JavaScript tests have no supported runner. Add repository-owned path mappings.'
    )
  }
  const nodeMappings = nodeMappingReview(projectPath, jsRunner)
  if (nodeMappings.blocker) blockers.push(nodeMappings.blocker)
  if (installs.includes(null)) {
    blockers.push(
      'Python tests have no supported install command. Add requirements.txt or a repository adapter.'
    )
  }
  if (managerSetup === null) {
    blockers.push(
      `The ${profile.packageManager} package manager needs a repository-owned setup adapter.`
    )
  }
  const workflows = workflowInventory(projectPath)
  if (
    workflows.some(
      workflow =>
        workflow.path !== WORKFLOW_PATH &&
        workflow.jobs.some(job => job.name === 'quality / required')
    )
  ) {
    blockers.push(
      'The quality / required status already exists. Choose a migration-safe context before generation.'
    )
  }

  return {
    schemaVersion: 1,
    policy,
    inventory: {
      packageManager: profile.packageManager,
      testScript: profile.scripts.test,
      jsRunner,
      python,
      packageManagerSetup: managerSetup || [],
      installCommands: installs.filter(Boolean),
      mappingSuggestions: nodeMappings.suggestions,
      runnerControlFiles: runnerControlFiles(projectPath, jsRunner, python),
      workflows,
    },
    blockers,
  }
}

function buildWorkflow(plan, runtimeSha) {
  if (!SHA_PATTERN.test(runtimeSha || '')) {
    throw new Error('runtimeSha must be an immutable 40-character commit SHA')
  }
  const auditCommands = plan.policy.audits[0]?.commands || []
  if (auditCommands.length === 0) {
    throw new Error('A declared complete test command is required')
  }
  if (plan.blockers.length > 0) {
    throw new Error(
      `Test-impact mappings are incomplete: ${plan.blockers.join(' ')}`
    )
  }
  const managerRun = plan.inventory.packageManagerSetup.join('\n          ')
  const installRun = plan.inventory.installCommands.join('\n          ')
  const policyDigest = crypto
    .createHash('sha256')
    .update(`${JSON.stringify(plan.policy, null, 2)}\n`)
    .digest('hex')
  const runnerControls = Buffer.from(
    JSON.stringify(plan.inventory.runnerControlFiles || [])
  ).toString('base64')
  return `# Generated by QA Architect. Preserve existing workflows during canary use.
name: Test impact

on:
  pull_request_target:
  schedule:
    - cron: '23 7 1 * *'
  workflow_dispatch:

permissions:
  contents: read

concurrency:
  group: test-impact-\${{ github.event.pull_request.number || github.ref }}
  cancel-in-progress: true

jobs:
  plan:
    name: Create trusted test plan
    runs-on: ubuntu-latest
    timeout-minutes: 5
    outputs:
      plan: \${{ steps.plan.outputs.plan }}
    steps:
      - name: Check out candidate
        uses: actions/checkout@${CHECKOUT_SHA} # v7.0.1
        with:
          repository: \${{ github.event.pull_request.head.repo.full_name || github.repository }}
          ref: \${{ github.event.pull_request.head.sha || github.sha }}
          path: candidate
          fetch-depth: 0
          persist-credentials: false
      - name: Check out protected policy
        uses: actions/checkout@${CHECKOUT_SHA} # v7.0.1
        with:
          repository: \${{ github.repository }}
          ref: \${{ github.event.pull_request.base.sha || github.sha }}
          path: protected
          persist-credentials: false
      - name: Check out the pinned planner
        uses: actions/checkout@${CHECKOUT_SHA} # v7.0.1
        with:
          repository: buildproven/claude-kit
          ref: ${runtimeSha}
          path: runtime
          persist-credentials: false
      - name: Set up Node.js
        uses: actions/setup-node@${SETUP_NODE_SHA} # v7.0.0
        with:
          node-version: '24'
      - name: Verify protected test controls
        if: github.event_name == 'pull_request_target'
        env:
          CANDIDATE_PACKAGE: candidate/package.json
          PROTECTED_PACKAGE: protected/package.json
          RUNNER_CONTROLS_BASE64: '${runnerControls}'
        run: |
          node - <<'NODE'
          const fs = require('node:fs')
          const readScripts = file => fs.existsSync(file)
            ? JSON.parse(fs.readFileSync(file, 'utf8')).scripts || {}
            : {}
          const candidate = readScripts(process.env.CANDIDATE_PACKAGE)
          const protectedScripts = readScripts(process.env.PROTECTED_PACKAGE)
          if (JSON.stringify(candidate) !== JSON.stringify(protectedScripts)) {
            throw new Error('Package scripts changed. Merge a reviewed policy update before this change.')
          }
          const controls = JSON.parse(Buffer.from(
            process.env.RUNNER_CONTROLS_BASE64,
            'base64'
          ).toString('utf8'))
          for (const file of controls) {
            const candidateControl = 'candidate/' + file
            const protectedControl = 'protected/' + file
            if (!fs.existsSync(candidateControl) ||
                !fs.existsSync(protectedControl) ||
                !fs.readFileSync(candidateControl).equals(fs.readFileSync(protectedControl))) {
              throw new Error('Runner control changed: ' + file + '. Merge a reviewed policy update first.')
            }
          }
          NODE
      - name: Create affected-test plan
        id: plan
        if: github.event_name == 'pull_request_target'
        working-directory: candidate
        env:
          BASE_SHA: \${{ github.event.pull_request.base.sha }}
          HEAD_SHA: \${{ github.event.pull_request.head.sha }}
        run: |
          if ! git cat-file -e "$BASE_SHA^{commit}" 2>/dev/null; then
            git fetch --no-tags --depth=1 "$GITHUB_SERVER_URL/$GITHUB_REPOSITORY.git" "$BASE_SHA"
          fi
          mapfile -d '' changed < <(git diff --name-only -z "$BASE_SHA" "$HEAD_SHA")
          plan=$(node ../runtime/scripts/test-impact.js --policy-root ../protected --policy-sha256 ${policyDigest} -- "\${changed[@]}" | jq -c .)
          if [ "$(jq -r .mode <<<"$plan")" = unmapped ]; then
            jq -r .remediation <<<"$plan" >&2
            exit 2
          fi
          echo "plan=$plan" >> "$GITHUB_OUTPUT"

  run-selected:
    name: Run selected tests
    if: github.event_name == 'pull_request_target' && needs.plan.result == 'success'
    needs: plan
    runs-on: ubuntu-latest
    timeout-minutes: 20
    steps:
      - name: Check out exact candidate
        uses: actions/checkout@${CHECKOUT_SHA} # v7.0.1
        with:
          repository: \${{ github.event.pull_request.head.repo.full_name }}
          ref: \${{ github.event.pull_request.head.sha }}
          persist-credentials: false
      - name: Set up Node.js
        uses: actions/setup-node@${SETUP_NODE_SHA} # v7.0.0
        with:
          node-version: '24'
      - name: Set up the declared package manager
        if: \${{ hashFiles('package.json') != '' }}
        run: |
          ${managerRun || 'true'}
      - name: Install declared dependencies without lifecycle scripts
        run: |
          ${installRun}
      - name: Run immutable selected commands
        env:
          TRUSTED_PLAN: \${{ needs.plan.outputs.plan }}
        run: |
          node - <<'NODE'
          const { spawnSync } = require('node:child_process')
          const plan = JSON.parse(process.env.TRUSTED_PLAN)
          for (const command of plan.commands || []) {
            const result = spawnSync(command.executable, command.args, {
              env: process.env,
              stdio: 'inherit',
            })
            if (result.error) throw result.error
            if (result.status !== 0) process.exit(result.status || 1)
          }
          NODE

  audit:
    name: Run complete selector audit
    if: github.event_name == 'schedule' || github.event_name == 'workflow_dispatch'
    runs-on: ubuntu-latest
    timeout-minutes: 20
    steps:
      - name: Check out protected branch
        uses: actions/checkout@${CHECKOUT_SHA} # v7.0.1
        with:
          persist-credentials: false
      - name: Set up Node.js
        uses: actions/setup-node@${SETUP_NODE_SHA} # v7.0.0
        with:
          node-version: '24'
      - name: Set up the declared package manager
        if: \${{ hashFiles('package.json') != '' }}
        run: |
          ${managerRun || 'true'}
      - name: Install declared dependencies without lifecycle scripts
        run: |
          ${installRun}
      - name: Run complete commands from protected policy
        env:
          TRUSTED_COMMANDS_BASE64: '${Buffer.from(JSON.stringify(auditCommands)).toString('base64')}'
        run: |
          node - <<'NODE'
          const { spawnSync } = require('node:child_process')
          const commands = JSON.parse(Buffer.from(
            process.env.TRUSTED_COMMANDS_BASE64,
            'base64'
          ).toString('utf8'))
          for (const command of commands) {
            const result = spawnSync(command.executable, command.args, {
              env: process.env,
              stdio: 'inherit',
            })
            if (result.error) throw result.error
            if (result.status !== 0) process.exit(result.status || 1)
          }
          NODE

  quality-required:
    name: quality / required
    if: always()
    needs: [plan, run-selected, audit]
    runs-on: ubuntu-latest
    steps:
      - name: Require the applicable test job
        run: |
          event='\${{ github.event_name }}'
          plan='\${{ needs.plan.result }}'
          selected='\${{ needs.run-selected.result }}'
          audit='\${{ needs.audit.result }}'
          if [ "$event" = pull_request_target ]; then
            [ "$plan" = success ] && [ "$selected" = success ]
          else
            [ "$audit" = success ]
          fi
`
}

function isOwnedGeneratedPair(policyFile, workflowFile) {
  if (!fs.existsSync(policyFile) || !fs.existsSync(workflowFile)) return false
  try {
    const policy = readJson(policyFile)
    const workflow = fs.readFileSync(workflowFile, 'utf8')
    return (
      policy.version === 1 &&
      workflow.startsWith('# Generated by QA Architect.')
    )
  } catch {
    return false
  }
}

function stageGeneratedOutputs(outputs) {
  return outputs.map(([file, content]) => {
    fs.mkdirSync(path.dirname(file), { recursive: true })
    const temporary = `${file}.qa-architect-${process.pid}.tmp`
    const backup = `${file}.qa-architect-${process.pid}.bak`
    fs.writeFileSync(temporary, content)
    const existed = fs.existsSync(file)
    if (existed) fs.copyFileSync(file, backup)
    return { file, temporary, backup, existed }
  })
}

function restoreGeneratedOutputs(staged) {
  for (const { file, backup, existed } of staged) {
    if (existed && fs.existsSync(backup)) fs.renameSync(backup, file)
    if (!existed && fs.existsSync(file)) fs.unlinkSync(file)
  }
}

function removeGeneratedTemporaryFiles(staged) {
  for (const { temporary, backup } of staged) {
    if (fs.existsSync(temporary)) fs.unlinkSync(temporary)
    if (fs.existsSync(backup)) fs.unlinkSync(backup)
  }
}

function replaceGeneratedOutputs(outputs, rename = fs.renameSync) {
  const staged = stageGeneratedOutputs(outputs)
  try {
    for (const { file, temporary } of staged) rename(temporary, file)
  } catch (error) {
    restoreGeneratedOutputs(staged)
    throw error
  } finally {
    removeGeneratedTemporaryFiles(staged)
  }
}

function writeGeneratedFiles(projectPath, plan, workflow, options = {}) {
  const policyFile = path.join(projectPath, POLICY_PATH)
  const workflowFile = path.join(projectPath, WORKFLOW_PATH)
  const policyContent = `${JSON.stringify(plan.policy, null, 2)}\n`
  const outputs = [
    [policyFile, policyContent],
    [workflowFile, workflow],
  ]
  const changed = outputs.some(([file, content]) => {
    return fs.existsSync(file) && fs.readFileSync(file, 'utf8') !== content
  })
  if (changed && options.update !== true) {
    throw new Error('Refusing to overwrite existing generated targets')
  }
  if (changed && !isOwnedGeneratedPair(policyFile, workflowFile)) {
    throw new Error('Refusing to update targets not owned by QA Architect')
  }
  replaceGeneratedOutputs(outputs, options.rename)
  return [POLICY_PATH, WORKFLOW_PATH]
}

module.exports = {
  POLICY_PATH,
  WORKFLOW_PATH,
  buildPolicy,
  buildWorkflow,
  writeGeneratedFiles,
  workflowInventory,
}
