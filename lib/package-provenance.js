...
function yarnModernConfig(source) {
  const parsed = yaml.load(source)
  if (!parsed || typeof parsed !== 'object' || Array.isArray(parsed)) {
    return { defaultRegistry: null, scopes: {} }
  }
  const config = { defaultRegistry: PUBLIC_NPM_REGISTRY, scopes: {} }
  if (Object.hasOwn(parsed, 'npmRegistryServer')) {
    config.defaultRegistry = registryValue(parsed.npmRegistryServer)
  }
  if (!parsed.npmScopes || typeof parsed.npmScopes !== 'object') return config
  for (const [rawScope, settings] of Object.entries(parsed.npmScopes)) {
    if (!settings || typeof settings !== 'object') continue
    if (!Object.hasOwn(settings, 'npmRegistryServer')) continue
    const scope = rawScope.startsWith('@') ? rawScope : `@${rawScope}`
    config.scopes[scope] = registryValue(settings.npmRegistryServer)
  }
  return config
}

function yarnClassicConfig(source) {
  const config = { defaultRegistry: PUBLIC_NPM_REGISTRY, scopes: {} }
  for (const rawLine of source.split(/\r?\n/)) {
    const line = rawLine.trim()
    ...
    if (key === 'registry') config.defaultRegistry = registryValue(value)
    else if (/^@[^:]+:registry$/.test(key)) {
      config.scopes[key.slice(0, key.indexOf(':'))] = registryValue(value)
    }
  }
  return config
}

function readYarnRegistryConfig(projectPath) {
  const modernPath = path.join(projectPath, '.yarnrc.yml')
  const classicPath = path.join(projectPath, '.yarnrc')
  try {
    if (fs.existsSync(modernPath)) {
      return yarnModernConfig(fs.readFileSync(modernPath, 'utf8'))
    }
    if (fs.existsSync(classicPath)) {
      return yarnClassicConfig(fs.readFileSync(classicPath, 'utf8'))
    }
    return { defaultRegistry: PUBLIC_NPM_REGISTRY, scopes: {} }
  } catch {
    return { defaultRegistry: null, scopes: {} }
  }
}

function resolveRegistryConfig(projectPath, packageNames, options = {}) {
  const packageManager =
    options.packageManager || detectPackageManager(projectPath)
  if (packageManager === 'yarn') return readYarnRegistryConfig(projectPath)
  return resolveNpmRegistryConfig(projectPath, packageNames, options)
}
...
function redactDependencySpec(rawSpec) {
  if (typeof rawSpec !== 'string') {
    const type = rawSpec === null ? 'null' : typeof rawSpec
    return `[invalid ${type}]`
  }
  const spec = rawSpec.trim()
  try {
    const url = new URL(
      spec.replace(/^([a-z][a-z\d+.-]*:\/\/)[^/@\s]+@/i, '$1[REDACTED]@')
    )
    if (!url.hostname) return spec
...
function excludedRecord(dependency, classification) {
  ...
  return {
    ...
    declaredSpec: redactDependencySpec(dependency.spec),
    resolvedName: classification.resolvedName,
    resolvedSpec: redactDependencySpec(classification.resolvedSpec),
  ...
  const lookedUp = await mapConcurrent(
    ...
    async item => {
      ...
      const record = {
        ...
        declaredSpec: redactDependencySpec(item.dependency.spec),
        resolvedName: item.classification.resolvedName,
        resolvedSpec: redactDependencySpec(item.classification.resolvedSpec),
        registry: item.classification.registry,
