...
function getSecurityScripts() {
  return {
    ...
    // Production deps only — dev-tooling CVEs don't ship to users, and gating
    // on them blocks releases on advisories with no consumer-side fix.
    'security:audit':
      '[ -f pnpm-lock.yaml ] && pnpm audit --audit-level high --prod || [ -f yarn.lock ] && yarn audit --level high --groups dependencies || npm audit --audit-level high --omit=dev',
    'security:audit:fix':
 ...
 */
function applySecurityFirstConfiguration(projectPath = '.', options = {}) {
  const securityFixes = []
  ...
  if (!options.preserveExistingLint && !fs.existsSync(eslintSecurityPath)) {
