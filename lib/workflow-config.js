...
function enforceMinimumNodeVersion(workflowContent, profile) {
  ...
  if (profile.nodeEngine) {
    ...
    minimumMajor = Math.max(minimumMajor, minimumVersion.major)
  }
  ...
  if (profile.packageManager === 'pnpm' && profile.packageManagerVersion) {
    ...
    if (pnpmMajor >= 11) minimumMajor = Math.max(minimumMajor, 22)
  }

  if (minimumMajor <= 20) return workflowContent

  return workflowContent
    .replace(/node-version: '20'/g, `node-version: '${minimumMajor}'`)
    .replace(/node-version: \[(?:20, )?22\]/g, `node-version: [${minimumMajor}]`)
}
