...
function injectProjectProfile(workflowContent, profile = {}) {
  ...
  const migrationTest = scripts.test
    ? `timeout 300 ${profile.runScript(scripts.test)}`
...
`
  ...
  updated = raiseNodeVersionForPnpm11(updated, profile)

  return updated
}

/**
 * pnpm 11 dropped support for Node <22 (it requires the node:sqlite
 * built-in, added in Node 22) — corepack activating a pinned pnpm@11.x
 * under a hardcoded 'Setup Node.js' node-version: '20' crashes every
 * non-matrix job outright with ERR_UNKNOWN_BUILTIN_MODULE. The `tests`
 * job's own matrix is left untouched here: it's already independently
 * configurable (minimal mode pins [22]; --matrix widens it), so consumers
 * who need older Node coverage there keep that choice — this only raises
 * the hardcoded non-matrix jobs (detect-maturity, security, audit-report,
 * documentation) to the minimum pnpm 11 actually requires.
 ...
 * @returns {string} Updated content
 */
function raiseNodeVersionForPnpm11(workflowContent, profile) {
  ...
  return workflowContent.replace(/node-version: '20'/g, "node-version: '22'")
}
