...
function detectSubmodulePaths(projectPath) {
  ...
  if (
    gitConfig.status !== 0 &&
    !(gitConfig.status === 1 && !gitConfig.stdout)
  ) {
    throw new Error(
      `Cannot parse .gitmodules: ${gitConfig.stderr || 'git config failed'}`.trim()
    )
  }
  const output = gitConfig.stdout.trim()
  ...
  if (packageManager === 'bun') {
    return {
      installCommand: hasLockfile
        ...
        : 'bun install',
      auditCommand: 'bun audit --audit-level=high --production',
      cacheManager: null,
    }
  }
  return {
