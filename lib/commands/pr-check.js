...
async function handlePrCheck(options = {}) {
  ...
  if (options.noFail) {
    process.stderr.write(
      '❌ --no-fail is incompatible with revision-bound PR assurance; the process status must match the verdict.\n'
    )
    process.exit(2)
  }
  ...
  const {
    ...
    toTerminal,
    writeEvidenceBundle,
  } = require('./pr-assurance')
  const projectPath = options.projectPath || process.cwd()
  const execution = await runPrAssurance(projectPath, options)
  if (options.json) {
    process.stdout.write(
      `${JSON.stringify(toJson(execution.result), null, 2)}\n`
    )
  } else if (options.sarif) {
    process.stdout.write(
      `${JSON.stringify(toSarif(execution.result), null, 2)}\n`
    )
  } else {
    process.stdout.write(`${toTerminal(execution.result)}\n`)
  }
  if (options.artifactDir) {
    const files = writeEvidenceBundle(options.artifactDir, execution)
    if (!options.json && !options.sarif) {
      process.stdout.write(
        `Evidence bundle: ${files.length} files in ${options.artifactDir}\n`
      )
    }
  }
  if (options.outPath) {
    fs.writeFileSync(
      options.outPath,
      `${toMarkdown(execution.result)}\n`,
      'utf8'
    )
  }
  process.exit(exitCode(execution.result.verdict))
}
