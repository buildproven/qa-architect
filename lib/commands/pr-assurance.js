...
const SCANNABLE = /\.(?:c|cc|cpp|cs|go|java|js|jsx|php|py|rb|rs|sql|ts|tsx)$/i
...
function selectedPackConfig(ruleRoot, selection, directory) {
  ...
  const rules = ruleFiles.flatMap(relative => {
    const parsed = yaml.load(
      fs.readFileSync(path.resolve(ruleRoot, '..', relative), 'utf8')
    )
  ...
  fs.writeFileSync(configPath, yaml.dump({ rules }, { lineWidth: 100 }))
  return configPath
}
...
function mapFinding(raw, projectPath, engineVersion = null) {
  ...
  const rawRuleId = String(raw.check_id || '')
    .split('.')
    .pop()
  const packCheck = checkForRule(rawRuleId)
  const severityMap = { ERROR: 'high', WARNING: 'medium', INFO: 'low' }
  return {
    source: 'semgrep',
    ruleId: canonicalRuleId(raw.check_id),
    ruleVersion: packCheck?.version || '1.0.0',
