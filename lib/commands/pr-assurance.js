...
function eligiblePaths(surface, policy) {
  ...
  return candidates.filter(filePath => !isExcluded(filePath, excludes))
}
...
function parseSemgrepResult(result, version) {
  try {
    ...
    if (
      ...
      return {
        outcome: 'partial',
        version,
        error: 'Semgrep reported incomplete analysis',
...
function defaultScanner(projectPath, paths, policy) {
  ...
  return parseSemgrepResult(result, version)
}
