...
function rejectReceiptOutputSymlinks(outputPaths) {
...
function writeReceiptBundle(directory, report) {
  fs.mkdirSync(directory, { recursive: true })
  const resolvedDirectory = requireReceiptDirectory(directory)
  const files = {
    'release-receipt.json': `${JSON.stringify(report, null, 2)}\n`,
    'release-receipt.md': buildMarkdown(report),
  }
  const outputPaths = Object.keys(files).map(name => path.join(directory, name))
  rejectReceiptOutputSymlinks(outputPaths)

  const token = `${process.pid}-${crypto.randomBytes(8).toString('hex')}`
  const temporaryPaths = Object.keys(files).map(name =>
    path.join(directory, `.${name}.${token}.tmp`)
  )
  let writeError = null
  try {
    ...
    if (
      ...
      fs.realpathSync(directory) !== resolvedDirectory
    ) {
      throw new Error(
        `Release Receipt artifact directory changed during write: ${directory}`
      )
    }
  ...
  throwReceiptWriteErrors(
    writeError,
    removeReceiptTemporaryFiles(temporaryPaths)
  )
  return outputPaths
}
...
function writeShipReport(report, options) {
  ...
  if (options.artifactDir) {
    writeReceiptBundle(options.artifactDir, report)
    if (!options.json) {
      process.stdout.write(
