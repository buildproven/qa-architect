...
async function runAudit(projectPath, options = {}) {
  ...
  return { findings, semgrepVersion, ruleFiles }
}
...
async function handleAudit(options = {}) {
  ...
  if ((wantFix || repairWith) && isPro) {
    ...
    for (const item of exports) {
      console.log(`📦 Remediation packet: ${item.path}`)
    }
    ...
    if (repairWith) {
      ...
      const evidence = orchestrateRemediation({
        ...
        allowedDirtyPaths: exports.map(exported =>
          path.relative(projectPath, exported.path).replaceAll('\\', '/')
        ),
      })
      ...
      console.log(`🧾 Remediation evidence: ${evidencePath}`)
      if (evidence.status !== 'VERIFIED') {
        console.error(`❌ Remediation incomplete: ${evidence.reason}`)
        process.exit(2)
      }
      console.log(`✅ Verified remediation commit: ${evidence.resultHead}`)
    }
  }
