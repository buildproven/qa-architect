...
function blockingIdentity(raw, worktreePath) {
  ...
  const relativePath = path.isAbsolute(rawPath)
    ? path.relative(worktreePath, rawPath).replaceAll('\\', '/')
    : rawPath.replace(/^\.\//, '')
  return `${raw.check_id}:${relativePath}:${raw.start?.line || 0}:${raw.end?.line || 0}`
}
...
function verifyRepair(context, packet, adapterName, ruleFiles, commandRunner) {
  ...
  const matchingBefore = before.findings.filter(raw =>
    rawFindingMatches(raw, packet, context.root)
  ).length
  const matchingAfter = after.findings.filter(raw =>
    rawFindingMatches(raw, packet, isolated)
  ).length
  if (matchingAfter >= matchingBefore) {
    return {
      evidence: incompleteEvidence(
        packet,
        adapterName,
