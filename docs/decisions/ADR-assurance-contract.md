...
- the Pro hallucinated-package registry engine is applicable and required only for a Pro-entitled `--audit` when a supported manifest declares dependencies eligible for verification against the public npm registry. A Free audit explicitly excludes this Pro engine from its supported-check scope and neither creates a required observation nor treats entitlement as missing evidence. The Pro engine resolves npm aliases to their target names, but excludes workspace/file/git specs and dependencies routed to private or custom registries. Exclusions remain visible coverage limitations and are not reported as missing packages. Every applicable registry observation requires explicit coverage counts, and a passed registry observation requires complete coverage; registry unavailability, timeout, malformed responses, missing coverage, or scanning fewer eligible packages than the engine claims to cover is partial required evidence and produces `INCOMPLETE`;
...
- Pro registry fixtures prove unavailable and partial/capped scans are `INCOMPLETE` when the engine applies, npm aliases resolve, and local/private/custom-registry specs become visible exclusions rather than false missing-package findings.
