...
| `orm-destructive-migration`          | deterministic | prisma, drizzle  | 1.0.0        | Use an expand-migrate-contract sequence with backup and rollback evidence.                 | Apply the migration to a production-like snapshot and verify preserved data and rollback. | A matched destructive operation may be intentional but remains release-sensitive. |
| `stripe-webhook-signature`           | heuristic     | stripe           | 1.0.0        | stripe.webhooks.constructEvent(rawBody, signature, webhookSecret)                          | Replay a payload with an invalid signature and confirm rejection before side effects.     | Custom verification wrappers may require a waiver after review.                   |
...
| `stripe-webhook-json-without-signature`     | high     | javascript, typescript | CWE-345  | A08:2021 | `.semgrep/vibe-moat-rules.yaml`    |
