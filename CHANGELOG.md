...
- **Trusted release verification:** the tag-driven release workflow now
  installs the pinned Semgrep CLI before running the prerelease suite, so the
  assurance-pack precision benchmark cannot be skipped on the publishing path.
...
- **Non-Git project setup:** submodule discovery now treats an ordinary project
  outside a Git worktree as having no submodules, while still rejecting a
  standalone `.gitmodules` file that cannot be validated against Git metadata.
- **Test isolation:** Python setup fixtures use unique temporary directories and
  clear inherited Git environment variables, preventing parent-worktree state
  from contaminating setup tests.
